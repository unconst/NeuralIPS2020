...
\begin{abstract}
The dominant tools used to measure and reward machine intelligence systems are benchmarks: datasets of labelled examples which demonstrate correct behavior for a set of specific predefined tasks. We argue that this reward mechanism is inefficient on account of its low-resolution and it's misalignment to general understanding. To reconcile these issues we propose BitTensor: a benchmark where intelligence is measured by other intelligence systems rather than against task-based tests. In this paper we describe how this novel benchmark is constructed and how it is negotiated by computers that share knowledge peer-to-peer (P2P) across the internet. However since this internet based negotiation introduces game-theoretic considerations, we design methods to ensure the veracity of the benchmark when peers remain self-interested and trustless. To test our design, we empirically show that the negotiation produces the desired ranking at the system's competitive equilibrium. The result is a benchmark which is simultaneously higher resolution and aligned to an arbitrarily general definition of intelligence. Furthermore, the system is inherently collaborative, open, decentralized, and can consume resources connected by an internet fabric.
